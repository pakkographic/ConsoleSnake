...
export async function deprecatePackages(options) {
    // Make a constant for all the registry JSON data sets
    const registryJsonFiles = [];
    // Iterate over all provided packages getting their data
    for (const pkg of options.package ?? []) {
        try {
            const registryData = await fetch(`${NpmBaseUrl}/${pkg.trim()}`, FetchResultTypes.JSON);
    ...
    for (const pkg of registryJsonFiles) {
        ...
        for (const version of Object.values(pkg.versions)) {
            if (!isNullish(version.deprecated))
                continue;
            if (version.version === pkg['dist-tags'].latest)
                continue;
            if (!options.deprecateDistTag && distTags.includes(version.version))
                continue;
        ...
        if (amountVersionsChanged >= 1) {
            fetchPromises.push(npmFetch(`/${encodeURIComponent(pkg.name).replace(/%40/, '@')}`, {
                body: pkg,
                fetchRetries: 2,
                fetchRetryFactor: 10,
